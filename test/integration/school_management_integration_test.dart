...
Map<String, String> _loadOfficialAssets() {
  ...
  for (final id in schoolFiles) {
    m['packages/taiyishenshu/assets/schools/$id.json'] = File('assets/schools/$id.json').readAsStringSync();
  }
  for (final id in deityFiles) {
    m['packages/taiyishenshu/assets/deities/$id.json'] = File('assets/deities/$id.json').readAsStringSync();
  }
  return m;
}
