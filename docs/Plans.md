...
六阶段计划中，第一至第五阶段已基本完成，第六阶段部分完成。

| 阶段 | 状态 | 说明 |
|------|------|------|
| 第一阶段：基础常量与模型 | 已完成 | 枚举/模型/输入输出已就位；`TaiYiRuleSet` 接口以 `_RuleProfile` 内部类形式实现；`TaiYiEngine` 入口以 `TaiYiPanCalculator` 形式实现 |
| 第二阶段：金镜派最小可用 | 已完成 | 金镜派四盘链路可运行 |
| 第三阶段：统宗派规则 | 已完成 | 统宗派四盘链路可运行，含五福盈差、三旗、贵神值事 |
| 第四阶段：集成派规则 | 已完成 | 集成派四盘链路可运行，含十二神计神、固定八门 |
| 第五阶段：UI 接入 | 已完成 | 日期选择 + 流派/盘类型/样式切换 + 三套网格 + 信息面板 |
| 第六阶段：测试与验盘 | 部分完成 | 年家金镜/统宗有测试覆盖；月/日/时家及集成派测试待补充 |

### 计划与实际差异

| 计划（Plans.md） | 实际实现 |
|-----------------|---------|
| `TaiYiRuleSet` 抽象接口 + 三个实现类 | `_RuleProfile` 内部类（数据驱动配置），因三派差异本质是参数差异 |
| `TaiYiEngine` 统一入口 | `TaiYiPanCalculator`，功能等价 |
| 按计算步骤拆分为多个 calculator 文件 | 集中在 `taiyi_pan_calculator.dart`（1740 行），暂未拆分 |
| `lib/taiyi/core/`, `rules/`, `calculators/` 子目录 | 扁平 `lib/taiyi/`，模型在 `lib/models/`，枚举在 `lib/enums/` |
